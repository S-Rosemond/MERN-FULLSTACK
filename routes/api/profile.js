...
router.get('/me', auth, async (req, res) => {
	try {
		...
		if (!profile) {
			return res.status(400).json({ msg: 'There is no profile associated with this account' });
		}
...
router.get('/github/:username', (req, res) => {
	const userClientID = config.get('githubClientId');
	const userSecret = config.get('githubSecret');
	try {
		const options = {
			uri: `https://api.github.com/users/${req.params
				.username}/repos?per_page=5&sort=created:asc&client_id=${userClientID}&client_secret=${userSecret}`,
			method: 'GET',
